...
docker compose exec -T ckan-dev ckan user add admin \
    fullname="Admin User" \
    email="admin@example.com" \
    password="admin" \
    2>/dev/null >&2 || echo "Admin user already exists" >&2

docker compose exec -T ckan-dev ckan sysadmin add admin 2>/dev/null >&2

# Generate API key for admin user
echo "🔑 Generating API key..." >&2
API_KEY=$(docker compose exec -T ckan-dev ckan user token add admin test-fixtures 2>/dev/null | awk 'NR>1 {print $1}')
